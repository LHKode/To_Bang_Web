...
if ( ! function_exists("base_url"))
{
    ...
    {
        return "http://".$_SERVER['SERVER_NAME'] .':8888/'. $url  ;
    }
}
    ...
    function recursive($listCategory ,$parents = 0 ,$level = 1 ,&$CategoryListSort)
    {
        if(count($listCategory) > 0 )
        {
            foreach ($listCategory as $key => $value) {
                if($value['danhmuc_id']  == $parents)
                {
                    $value['level'] = $level;
                    $CategoryListSort[] = $value;
                    unset($listCategory[$key]);
                    $newparents = $value['id_danhmuc'];
...
function curPageURL() {
    ...
    $pageURL = preg_split("/\?|index.php/",$pageURL);
